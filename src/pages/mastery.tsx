...
export default function Mastery() {
      ...
      {loading ? (
        ...
        <div className="space-y-1">
          {masteries.map((m) => {
            ...
            const color = MASTERY_COLOR[lvl] ?? MASTERY_COLOR[Math.min(lvl, 10)] ?? '#5B5A56';
  ...
  return (
    <div className="p-6 space-y-5 animate-slide-up">
      ...
      {/* Content — both tabs show the same champion list, 
           Overview could show top 10, Champions shows all */}
      {activeTab === 'Overview' && (
        <>
          {!loading && masteries.length > 0 && (
            <p className="text-xs text-ink-ghost">Showing top 10 champions</p>
          )}
          {loading ? content : (
            <div className="space-y-1">
